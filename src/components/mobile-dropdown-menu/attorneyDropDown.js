...
export default function AttorneyDropDown(props) {
  ...
  useEffect(() => {
    attorneys.forEach((attorney) => {
      const href = `/attorneys/${
        attorney.substring(0, 1) == "j"
          ? attorney.split(", ")[0].split(" ").pop()
          : attorney.split(" ").pop()
      }`;
      if (router.pathname === href) {
        setSelectedItem(attorney.toUpperCase());
        setIsClicked(true);
      }
    });
  }, [router.pathname]);

  const getAttorneySlug = (attorney) => {
    return `/attorneys/${
      attorney.substring(0, 1) == "j"
        ? attorney.split(", ")[0].split(" ").pop()
        : attorney.split(" ").pop()
    ...
    <DropDown ref={closeRef} size={props?.size} padding={props?.padding}>
      ...
      <ul className={`drop-down__body ${isOpen ? "open" : ""}`}>
        {attorneys.map((attorney, index) => {
          return (
            <Link href={getAttorneySlug(attorney)} key={index}>
              <li
                className="drop-down__body__item"
                key={index}
                onClick={() => handleClick(attorney.toUpperCase())}
              >
